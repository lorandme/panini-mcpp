...
int main() {
    ...
    // URL-ul serverului
    std::string serverUrl = "http://localhost:8080";  // Asigură-te că serverul rulează pe acest port

    // Crearea instanței clientului
    Client client(serverUrl);

    int choice;
    std::cout << "Alegeți opțiunea:\n";
    std::cout << "1. Înregistrare\n";
    ...
    if (choice == 1) {
        ...
        if (result) {
            std::cout << "Utilizatorul a fost inregistrat cu succes." << std::endl;
        }
        else {
            std::cout << "Nu s-a putut inregistra utilizatorul." << std::endl;
        }
    }
    else if (choice == 2) {
        ...
        if (result) {
            ...
            std::cout << "Autentificare eșuată!" << std::endl;
        }
    }
    else {
